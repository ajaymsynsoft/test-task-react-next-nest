...
export default function FileField<TFieldValues extends FieldValues, TName extends FieldPath<TFieldValues>>(props: FileFieldProps<TFieldValues, TName>) {
  ...
  return (
    <Controller
      ...
      render={({ fieldState: { error }, field: { ref, onChange, value, ...restField } }) => {
        ...
        const link = value ? ((value as any) instanceof File ? URL.createObjectURL(value as File) : value) : ''

        return (
          <FormControl fullWidth error={!!error}>
            {label && <FormLabel sx={{ lineHeight: 1, mb: 0.75, mt: -0.25 }}>{label}</FormLabel>}
